    ...
    try {
      ...
      if (typeof sheetIndexOrName === "number") {
        if (sheetIndexOrName >= workbook.SheetNames.length) {
          console.error(
            `Sheet index ${sheetIndexOrName} out of range. Max index: ${workbook.SheetNames.length - 1}`,
          );
          return null;
        }
        ...
        if (!workbook.SheetNames.includes(sheetIndexOrName)) {
          console.error(
            `Sheet name "${sheetIndexOrName}" not found in the workbook.`,
          );
          return null;
        }
